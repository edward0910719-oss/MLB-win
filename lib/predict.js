...
function buildCommentary(home, away, homeProb, contributions) {
  ...
  const favoredPct = Math.round(Math.max(homeProb, 1 - homeProb) * 100);

  if (ranked.length === 0) {
    return `雙方各項數據相當接近，模型計算後${favored.zh}僅以些微優勢領先（${favoredPct}%），比賽走向不確定性較高。`;
  }
  ...
  let text = `${favored.zh}獨贏機率${favoredPct}%`;
  if (forFavored.length > 0) text += `，主要受惠於${forFavored.join("、")}`;
