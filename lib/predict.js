...
// Plain-language summary of the total-runs (O/U) call — unlike the win-probability
// commentary above, this isn't ranked off internal z-contributions (the run-total math
// doesn't route through the same weighted-sum structure); instead it checks the same
// handful of run-environment inputs the model actually uses (combined starter ERA vs
// league average, home park factor, forecast temperature) against thresholds picked to
// flag only genuinely notable conditions, not everyday variation.
function buildOuCommentary(g, home, hpEra, apEra, leagueAvgEra, isOver, line) {
  ...
  if (eraDiff <= -0.4) reasons.push("雙方先發投手ERA都優於聯盟平均，比賽可能偏低比分");
  else if (eraDiff >= 0.4) reasons.push("雙方先發投手ERA都高於聯盟平均，容易失分偏多");

  if (home.parkFactor >= 105) reasons.push(`${home.zh}主場球場因素偏向打者有利（${home.parkFactor}）`);
  else if (home.parkFactor <= 95) reasons.push(`${home.zh}主場球場因素偏向投手有利（${home.parkFactor}）`);

  if (g.weatherTempF !== null) {
    if (g.weatherRunFactor >= 1.03) reasons.push("預估氣溫偏高，有利長打");
    else if (g.weatherRunFactor <= 0.97) reasons.push("預估氣溫偏低，不利長打");
  }

  const dirLabel = isOver ? "大" : "小";
  if (reasons.length === 0) {
    return `大小分預測${dirLabel}${line}，雙方先發戰力與球場條件接近聯盟平均，屬中性判斷。`;
  }
  return `大小分預測${dirLabel}${line}，主要考量：${reasons.join("；")}。`;
}
